...
def download(url,number):
    ...
    if not os.path.exists(f"{cwd}\\Result\{fold}\{number}"):
        os.makedirs(f"{cwd}\\Result\\{fold}\{number}")
    ...
    sleep(2)
    url_href = wait(browser,15).until(EC.presence_of_element_located((By.XPATH, f'(//a[@class="btn btn-md btn-success"])[2]'))).get_attribute('href')
    id_vid = url.split("video/")
    id_vid = id_vid[1]
    results= requests.get(f"{url_href}", headers=headers)
    
    with open(f"{cwd}\\Result\\{fold}\\{number}\\{id_vid}.mp4", 'wb') as f:
